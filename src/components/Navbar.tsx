...
export const Navbar = () => {
  ...
  const handleContactClick = (e: React.MouseEvent) => {
    ...
    // Update hash manually if needed
    window.history.pushState(null, '', '#contact');
  };

  return (
    <>
      <nav className="fixed top-0 left-0 w-full p-6 md:p-10 z-[100] flex justify-between items-center mix-blend-difference text-white pointer-events-none">
        <div className="pointer-events-auto hover-trigger">
          <Link to="/" className="font-serif font-bold text-2xl tracking-tighter">JNY SC.</Link>
        </div>
        <button 
          ...
          >
            ...
            >
              <svg width="120" height="60" viewBox="0 0 120 60" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M10 30 Q 35 5, 60 30 T 110 30" stroke="#FF1493" strokeWidth="6" strokeLinecap="round" />
              </svg>
            </motion.div>
            <motion.div 
              initial={{ opacity: 0, scale: 0 }}
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center relative z-10">
              <ul className="flex flex-col gap-2 md:gap-6 w-full">
                {navItems.map((item, i) => {
                  const isActive = item.name !== 'Contact' && (item.isHash 
                    ? (isHome && location.hash === `#${item.path}`)
                    : location.pathname === item.path);
                  
                  const isHovered = hoveredItem === item.name;
                  const isFaded = hoveredItem !== null && hoveredItem !== item.name;
                  ...
                  return (
                    ...
                    >
                      <div className={`group flex items-center gap-6 transition-all duration-500 ${isFaded ? 'opacity-20' : 'opacity-100'}`}>
                        {/* Interactive Link */}
                        <div className="relative flex items-center">
                          ...
                          {item.name === 'Contact' ? (
                            <a 
                              ...
                              className="font-serif font-black tracking-tighter text-5xl md:text-8xl transition-all duration-500 hover-trigger inline-block relative"
                              style={{ color: isHovered ? item.color : '#1a1a1a' }}
                            >
                            ...
                            isHome ? (
                              <a 
                                ...
                                className="font-serif font-black tracking-tighter text-5xl md:text-8xl transition-all duration-500 hover-trigger inline-block relative"
                                style={{ color: (isHovered || isActive) ? item.color : '#1a1a1a' }}
                              >
                                <span className="relative z-10">{item.name}</span>
                              </a>
                            ) : (
                        ...
                        >
                          ...
                          <span className="font-hand text-2xl" style={{ color: item.color }}>
                            {item.desc}
                          </span>
                        </motion.div>
                      </div>
              ...
              <div className="hidden lg:block w-1/3 text-right">
                ...
                >
                  <p className="font-hand text-2xl mb-2 text-black">"Making memories</p>
                  <p className="font-hand text-2xl text-black">one event at a time!"</p>
                  <div className="mt-4 flex justify-end">
                    <div className="w-12 h-12 bg-[#FF1493] rounded-full border-2 border-black flex items-center justify-center">
                      <span className="font-serif font-bold text-xl text-white">★</span>
                    </div>
                  </div>
                </motion.div>
              </div>
